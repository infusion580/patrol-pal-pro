...
const ChatRH = () => {
  ...
  const [messages, setMessages] = useState<Array<{ id: number; text: string; sender: 'me' | 'rh'; time: string; confidential?: boolean }>>([]);
          ...
          <div className="space-y-2">
            {topics.map(topic => (
              <button
                ...
                className="w-full bg-card rounded-xl p-4 shadow-card text-left hover:shadow-elevated transition-shadow active:scale-[0.98] flex items-center gap-3"
              >
      ...
      <div className="bg-primary text-primary-foreground px-4 pt-12 pb-4">
        <div className="max-w-lg mx-auto flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs opacity-70">Folio: RH-{Date.now().toString().slice(-6)}</p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2 mb-2">
            <button
              ...
              className={`flex items-center gap-1 text-xs px-2 py-1 rounded-full transition-colors ${
                isConfidential ? 'bg-warning/10 text-warning' : 'bg-muted text-muted-foreground'
              }`}
            >
